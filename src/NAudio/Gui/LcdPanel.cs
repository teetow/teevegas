...
namespace NAudio.Gui
{
	...
	/// LCD panel control
	/// </summary>
	public class LcdPanel : UserControl
	{
		/// <summary> 
...
#pragma warning restore 649
		...
		public LcdPanel()
		{
			...
			BackColor = Color.FromArgb(0xBD, 0xFF, 0xBC); //Color.FromArgb(0x8D,0xE0,0x74);
			OffColor = Color.FromArgb(0x7D, 0xC6, 0x67);
			OnColor = Color.FromArgb(0x42, 0x68, 0x37);
			Text = "ACDEFHILMNORSTU";
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			int row = 0;
			int col = 0;
			int charsPerLine = Width/(pixelSize*6);
			int lines = Height/(pixelSize*8);
		...
		private void DrawCharacter(Graphics g, char c, int row, int col)
		{
			...
			var onBrush = new SolidBrush(OnColor);
			for (int x = 0; x < 5; x++)
				for (int y = 0; y < 7; y++)
					if (b[y*5 + x])
						g.FillRectangle(onBrush, 1 + col*6*pixelSize + x*pixelSize, 1 + row*8*pixelSize + y*pixelSize, pixelSize,
						                pixelSize);
		}
	}
}
...
namespace NAudio.FileFormats.Ogg
{
	...
	public class OggEncoder : IDisposable
	{
		...
		private readonly IntPtr og; // ogg_page one Ogg bitstream page.  Vorbis packets are inside

		private readonly IntPtr op; // ogg_packet one raw packet of data for decode
		private readonly IntPtr os; // ogg_stream_state take physical pages, weld into a logical stream of packets
		private readonly IntPtr vb; // vorbis_block local working space for packet->PCM decode
		private readonly IntPtr vc; // vorbis_comment struct that stores all the user comments
		private readonly IntPtr vd; // vorbis_dsp_state central working state for the packet->PCM decoder
		private readonly IntPtr vi; // vorbis_info struct that stores all the static vorbis bitstream settings
		...
		private IntPtr AllocateHGlobal(int bytes)
		{
			...
			var ret = new IntPtr(memPool.ToInt32() + memPos);
			memPos += bytes;
		...
		~OggEncoder()
		{
			Debug.Assert(true, "Ogg encoder wasn't disposed");
			Dispose(false);
		}

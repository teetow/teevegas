	...
	internal class AcmStream : IDisposable
	{
		...
		public int SourceToDest(int source)
		{
			if (source == 0) // zero is an invalid parameter to acmStreamSize
				return 0;
			int convertedBytes;
			MmException.Try(AcmInterop.acmStreamSize(streamHandle, source, out convertedBytes, AcmStreamSizeFlags.Source),
			                "acmStreamSize");
		...
		public int DestToSource(int dest)
		{
			if (dest == 0) // zero is an invalid parameter to acmStreamSize
				return 0;
			int convertedBytes;
			MmException.Try(AcmInterop.acmStreamSize(streamHandle, dest, out convertedBytes, AcmStreamSizeFlags.Destination),
			                "acmStreamSize");
			return convertedBytes;
		}
		...
		public static WaveFormat SuggestPcmFormat(WaveFormat compressedFormat)
		{
			...
			MmException.Try(
				AcmInterop.acmFormatSuggest(IntPtr.Zero, compressedFormat, suggestedFormat, Marshal.SizeOf(suggestedFormat),
				                            AcmFormatSuggestFlags.FormatTag), "acmFormatSuggest");
		...
		public int Convert(int bytesToConvert, out int sourceBytesConverted)
		{
			...
			{
				Debug.WriteLine(String.Format("NOT A WHOLE NUMBER OF BLOCKS", bytesToConvert));

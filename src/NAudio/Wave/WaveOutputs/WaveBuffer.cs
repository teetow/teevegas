...
namespace NAudio.Wave.WaveOutputs
{
	...
	public class WaveBuffer : IWaveBuffer
	{
		...
		[FieldOffset(4)] private byte[] byteBuffer;
		[FieldOffset(4)] private float[] floatBuffer;
		[FieldOffset(4)] private short[] shortBuffer;
		[FieldOffset(4)] private int[] intBuffer;
		...
		private int CheckValidityCount(string argName, int value, int sizeOfValue)
		{
			...
			if ((newNumberOfBytes%4) != 0)
			{
				throw new ArgumentOutOfRangeException(argName,
				                                      String.Format("{0} cannot set a count ({1}) that is not 4 bytes aligned ",
				                                                    argName, newNumberOfBytes));
			}

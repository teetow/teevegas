...
namespace NAudio.Wave.WaveOutputs
{
	...
	public class WaveFileWriter : IDisposable
	{
		...
		private readonly bool overwriting;
		...
		[Obsolete(
			"Not planning to keep supporting this, should create derived WaveFileWriter for this type of behaviour if needed")]
		...
		public void WriteData(short[] data, int offset, int count)
		{
			...
			else if (WaveFormat.BitsPerSample == 24)
			{
				...
				for (int sample = 0; sample < count; sample++)
				{
					value = BitConverter.GetBytes(UInt16.MaxValue*data[sample + offset]);
					value24[0] = value[1];
					value24[1] = value[2];
					value24[2] = value[3];
					writer.Write(value24);
			...
			else if (WaveFormat.BitsPerSample == 32 && WaveFormat.Encoding == WaveFormatEncoding.Extensible)
			{
				for (int sample = 0; sample < count; sample++)
				{
					writer.Write(UInt16.MaxValue*data[sample + offset]);
				}
			...
			else if (WaveFormat.BitsPerSample == 32 && WaveFormat.Encoding == WaveFormatEncoding.IeeeFloat)
			{
				for (int sample = 0; sample < count; sample++)
				{
					writer.Write(data[sample + offset]/(float) (Int16.MaxValue + 1));
				}
				dataChunkSize += (count*4);
			}
			else
